...
final class MqttClient {
    ...
    func start(host: String, port: UInt16, username: String, password: String) {
        queue.async {
            let credentials = "\(host)\0\(port)\0\(username)\0\(password)"
            if !self.stopped, self.activeCredentials == credentials { return }
            self.stopLocked()
            guard !host.isEmpty, port > 0, !username.isEmpty, !password.isEmpty else {
