...
private let defaultMqttHost = "b01a87f3.ala.cn-hangzhou.emqxsl.cn"
private let defaultMqttPort: UInt16 = 8883
...
struct BridgeConfiguration: Codable {
    ...
    static func loadOrCreate() throws -> BridgeConfiguration {
        ...
        if configuration.mqttHost?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty != false {
            configuration.mqttHost = defaultMqttHost
        }
        if configuration.mqttPort == nil || configuration.mqttPort == 0 {
            configuration.mqttPort = defaultMqttPort
        }
...
final class StatusMenu {
    ...
    init(
        ...
        onSaveMqtt: @escaping (String, UInt16, String, String) -> Void
    ) {
        self.historyStore = historyStore
        self.launcher = launcher
        mqttSettingsWindow = MqttSettingsWindowController(
            host: configuration.mqttHost ?? defaultMqttHost,
            port: configuration.mqttPort ?? defaultMqttPort,
        ...
        item.button?.image = NSImage(
            ...
            accessibilityDescription: "拾光快捷工具"
        )
        let menu = NSMenu()
        menu.addItem(NSMenuItem(title: "拾光快捷工具", action: nil, keyEquivalent: ""))
...
final class MqttSettingsWindowController: NSWindowController, NSWindowDelegate {
    init(
        ...
        username: String,
        password: String,
        onSave: @escaping (String, UInt16, String, String) -> Void
    ) {
        let window = NSWindow(
        ...
        window.contentView = MqttSettingsView(
            ...
            username: username,
            password: password,
            onSave: { [weak window] savedHost, savedPort, savedUsername, savedPassword in
                onSave(savedHost, savedPort, savedUsername, savedPassword)
                window?.close()
            }
        )
        window.center()
...
final class MqttSettingsView: NSView {
    ...
    private let onSave: (String, UInt16, String, String) -> Void
    ...
    init(
        ...
        self.onSave = onSave
        super.init(frame: NSRect(x: 0, y: 0, width: 440, height: 370))

        let title = NSTextField(labelWithString: "跨网络同步")
        title.font = .boldSystemFont(ofSize: 20)
        title.frame = NSRect(x: 28, y: 319, width: 384, height: 26)
        addSubview(title)
        ...
        detail.frame = NSRect(x: 28, y: 280, width: 384, height: 34)
        addSubview(detail)

        hostField.placeholderString = "MQTT 服务器地址"
        hostField.stringValue = host
        hostField.frame = NSRect(x: 28, y: 229, width: 282, height: 28)
        addSubview(hostField)

        portField.placeholderString = "端口"
        portField.stringValue = String(port)
        portField.frame = NSRect(x: 322, y: 229, width: 90, height: 28)
        addSubview(portField)

        usernameField.placeholderString = "MQTT 用户名"
        usernameField.stringValue = username
        usernameField.frame = NSRect(x: 28, y: 181, width: 384, height: 28)
        addSubview(usernameField)

        passwordField.placeholderString = "MQTT 密码"
        passwordField.stringValue = password
        passwordField.frame = NSRect(x: 28, y: 139, width: 384, height: 28)
    ...
    @objc private func save() {
        ...
        guard !host.isEmpty, let port = UInt16(portField.stringValue), port > 0 else {
            NSSound.beep()
            return
        }
...
final class ClipboardBridge {
    ...
    init(
        ...
        mqttHost = configuration.mqttHost ?? defaultMqttHost
        mqttPort = configuration.mqttPort ?? defaultMqttPort
    ...
    func updateMqtt(host: String, port: UInt16, username: String, password: String) {
        queue.async {
            self.mqttHost = host
            self.mqttPort = port
...
do {
    ...
    // Install the global monitor after AppKit has entered its run loop. This
    // makes the first double-Command work even before the launcher is opened.
    ...
    let menu = StatusMenu(
        ...
        launcher: launcher,
        onSaveMqtt: { host, port, username, password in
            configuration.mqttHost = host
            configuration.mqttPort = port
    ...
    bridge?.start()
    withExtendedLifetime((menu, bridge, launcher, commandMonitor)) {
